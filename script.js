...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const dataTableContainer = dataTable.parentElement;
    const rowHeight = 40; // Height of each row in px
    const buffer = 5; // Buffer rows above and below the viewport
    let totalRows = 0;
    let data = [];

    const calculateVisibleRowCount = () => Math.ceil(dataTableContainer.clientHeight / rowHeight);
    let visibleRowCount = calculateVisibleRowCount();

    const renderRows = (startIndex) => {
        const fragment = document.createDocumentFragment();
        const endIndex = Math.min(startIndex + visibleRowCount + buffer * 2, totalRows);

        dataTable.querySelector('tbody').innerHTML = '';

        for (let i = startIndex - buffer; i < endIndex; i++) {
            if (i >= 0 && i < totalRows) {
        ...
        dataTable.querySelector('tbody').appendChild(fragment);
    };

    dataTableContainer.addEventListener('scroll', () => {
        const scrollTop = dataTableContainer.scrollTop;
        const firstVisibleRowIndex = Math.floor(scrollTop / rowHeight);
        renderRows(firstVisibleRowIndex);
    });
    ...
    // Expose addRow function to global scope for adding rows dynamically
