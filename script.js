document.addEventListener("DOMContentLoaded", function () {
  ...
  form.addEventListener("submit", function (event) {
    ...
    var existingEntry = dataTableBody.querySelector(
      "tr[data-date='" + date + "']"
    );
    if (existingEntry) {
    ...
    var totalAssigned = parseInt(
      document.getElementById("totalAssigned").value
    );
    var totalDelivered = parseInt(
      document.getElementById("totalDelivered").value
    );
  ...
  function updateSummary() {
    ...
    rows.forEach(function (row) {
      ...
      var moneyEarned = parseInt(
        row.querySelector("td:nth-child(7)").textContent
      );
      var moneyDifference = parseInt(
    ...
    document.getElementById("totalMoneyDifference").textContent =
...
function closeNavBody() {
  ...
  document.getElementById("weekly-transactions-body").style.visibility =
    "hidden";
  document.getElementById("weekly-transactions-body").style.transform =
    "translate(200%, -40%)";
}
...
function updateTodaysNetIncome() {
  ...
  var tableBody = document
    ...
    .getElementsByTagName("tbody")[0]; // Get the tbody element
...
function displayWeeklyTransactions() {
  var dataTableBody = document.querySelector("#dataTable tbody");
  var weeklyTransactionsCnt = document.querySelector(
    ".weekly-transactions-cnt"
  );
  ...
  for (var weekNumber in weeklyTotals) {
    if (weeklyTotals.hasOwnProperty(weekNumber)) {
      ...
      weekDiv.innerHTML +=
        "<p class='total-income'>Total Income: <span class='weeks-total-income'>" +
        weekData.income +
        "</span></p>" +
        "<p class='total-petrol'>Total Petrol: <span class='weeks-total-petrol'>" +
        weekData.petrol +
        "</span></p>" +
        "<p class='net-difference'>Net Difference: <span class='weeks-total-difference'>" +
        weekData.difference +
        "</span></p>";
...
function getWeekNumber(date) {
  ...
  var daysOffset = Math.floor(
    (firstDayOfWeek - januaryFirst) / (24 * 60 * 60 * 1000)
  );
...
function getStartDateOfWeek(weekNumber) {
  ...
  var firstDayOfWeek = new Date(
    januaryFirst.getTime() + daysOffset * 24 * 60 * 60 * 1000
  );
...
function openGraphs() {
  ...
  document.getElementById("nav-bar-graph-data-cnt").style.background =
    "var(--light-grad-pink-blue, radial-gradient(ellipse at 40% 0%, #bf398910 0, transparent 75%), radial-gradient(ellipse at 60% 0%, #096bde10 0, transparent 75%))";
  ...
  document.getElementById("earningsChartContainer").style.visibility =
    "visible";
  document.getElementById("earningsChartContainer").style.transform =
    "translate(0,0)";
  document.getElementById("earningsChartContainer").style.fontSize = "1em";
...
function closeGraphs() {
  ...
  document.getElementById("nav-bar-graph-data-cnt").style.transform =
    "translate(0,0)";
  document.getElementById("nav-bar-graph-data-cnt").style.background = "";
...
userNameInput.value =
  localStorage.getItem("userName") || userNameTitle.textContent;
userOccupationInput.value =
  localStorage.getItem("userOccupation") || userOccupationTitle.textContent;
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  downloadPdfButton.addEventListener("click", function () {
    ...
    // Get the text content from the elements
    const userNameTitleId =
      document.getElementById("UserName-Title-Id").textContent;
    const abtMeOccTitle =
      document.getElementById("abtme-occ-title").textContent;
